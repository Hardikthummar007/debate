...
function DebateRoom() {
  ...
  const [wagerPools, setWagerPools] = useState(null);
  ...
  const isSpectator = session ? (String(user.id) !== String(session.participantA.id) && (session.participantB ? String(user.id) !== String(session.participantB.id) : true)) : false;

  // Poll active wagers to sync live pools
  useEffect(() => {
    ...
    const fetchLivePools = async () => {
      try {
        ...
        const myMatch = res.find(m => String(m.id) === String(id));
        if (myMatch) {
          setWagerPools(myMatch);
        }
      } catch (e) {
        console.error("Failed to load live pools", e);
      }
    };
  ...
  // Load User's Bet History for this match
  useEffect(() => {
    if (!id || !isSpectator) return;

    const fetchUserBetHistory = async () => {
      try {
        const history = await wagerApi.getHistory();
        const existingBet = history.find(b => String(b.matchId) === String(id));
        if (existingBet) {
          setMyActiveBet(existingBet);
        }
      } catch (e) {
        console.error("Failed to load user bet history", e);
      }
    };
  ...
  const handlePlaceWager = async (side) => {
    if (betAmount < 10) {
      setBetError("Minimum wager is 10 gold coins.");
      return;
    }
    setBetError('');
    setBetSuccess('');
    setPlacingBet(true);
    try {
      const res = await wagerApi.placeBet(id, side, betAmount);
      setBetSuccess("Wager registered in the Citadel ledger!");
      // Reload history
      const history = await wagerApi.getHistory();
      const existingBet = history.find(b => String(b.matchId) === String(id));
      if (existingBet) {
      ...
      // Reload pools
      const pools = await wagerApi.getActiveWagers();
      const myMatch = pools.find(m => String(m.id) === String(id));
      if (myMatch) {
        setWagerPools(myMatch);
      }
    } catch (err) {
        ...
        setAiStreamingProgress(prev => {
          ...
          switch (event) {
            case 'ddg_start':
              ...
              break;
            case 'ddg_complete':
              next.ddg = { status: 'completed', detail: data.message, strategy: data.strategy };
              break;
            case 'judge3_start':
  ...
  // Normalize arguments list for uniform rendering (supporting both java session and room structures)
  session.arguments = [];
  if (session.rounds) {
      ...
      if (r.arguments) {
        r.arguments.forEach(arg => {
          session.arguments.push({
            ...
            userId: arg.user ? String(arg.user.id) : String(arg.userId),
            username: arg.user ? arg.user.username : arg.username,
  ...
  let hostHP = 100;
  let guestHP = 100;
  if (session && session.arguments) {
    const roundArgs = {};
    session.arguments.forEach(arg => {
      if (!roundArgs[arg.roundNumber]) roundArgs[arg.roundNumber] = [];
      roundArgs[arg.roundNumber].push(arg);
    });
    Object.values(roundArgs).forEach(args => {
      if (args.length === 2) {
        let scoreA = 0; // Participant A
        let scoreB = 0; // Participant B
        args.forEach(arg => {
          if (arg.userId === session.participantA.id) scoreA = arg.score;
          else scoreB = arg.score;
        });
        if (scoreA > scoreB) {
          guestHP -= (scoreA - scoreB);
        } else {
          hostHP -= (scoreB - scoreA);
        }
      }
    });
  }
  hostHP = Math.max(20, Math.min(100, hostHP));
  guestHP = Math.max(20, Math.min(100, guestHP));

  const hostStance = session.userAStance;
  const guestStance = session.userBStance;
  const actualIsMyTurn = session.currentTurnUserId === user.id;

  const renderPlayerColumn = (pId, pName, pStance, pHP, pScore, isCurrentTurn) => {
    const roundArg = session.arguments.find(a => a.roundNumber === session.currentRound && String(a.userId) === String(pId));
    const isHumanBeingEvaluated = streamingProgress && String(pId) === String(session.participantA.id);
    const isAiBeingEvaluated = aiStreamingProgress && String(pId) === String(session.participantB.id);
    const isBeingEvaluated = isHumanBeingEvaluated || isAiBeingEvaluated;
    const activeEvalData = isHumanBeingEvaluated ? streamingProgress : aiStreamingProgress;
    
    const stanceColor = pStance === 'FOR' ? 'var(--for-color)' : 'var(--against-color)';
    const stanceBg = pStance === 'FOR' ? 'rgba(16,185,129,0.08)' : 'rgba(239,68,68,0.08)';

    return (
      <div className={`royal-card p-6 flex flex-col gap-4 relative transition-all duration-300 ${isCurrentTurn ? 'ring-2 ring-gold' : 'border-white/5 bg-black/20'}`} style={{ border: isCurrentTurn ? '1px solid var(--accent)' : undefined }}>
        {/* Profile / Status Header */}
        <div className="flex items-center justify-between border-b border-white/5 pb-3">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-full border border-gold/30 bg-cover bg-center shrink-0 flex items-center justify-center text-xl bg-black/40">
              {pStance === 'FOR' ? '🛡️' : '🔥'}
            </div>
            <div>
              <div className="font-display text-sm text-white font-semibold flex items-center gap-1.5">
                {pName}
                {String(pId) === String(session.participantA.id) && <span className="text-[10px] text-gold font-normal">(Host)</span>}
              </div>
              <div className="font-display text-[9px] px-2 py-0.5 rounded-sm uppercase tracking-wider inline-block mt-1" style={{ color: stanceColor, background: stanceBg, border: `1px solid ${stanceColor}20` }}>
                House {pStance}
              </div>
            </div>
          </div>
          <div className="text-right">
            <div className="font-display text-xs text-gold font-bold">{pScore.toFixed(1)} PTS</div>
            <div className="font-body text-[10px] text-muted-foreground mt-0.5">{Math.round(pHP)} HP</div>
          </div>
        </div>

        {/* HP Bar */}
        <div className="hp-bar w-full h-2 bg-black/50 border border-white/5 rounded-full overflow-hidden">
          <div 
            className="hp-bar-fill h-full transition-all duration-500" 
            style={{ 
              width: `${pHP}%`, 
              background: pStance === 'FOR' 
                ? 'linear-gradient(90deg, var(--for-color), rgba(16,185,129,0.5))' 
                : 'linear-gradient(90deg, var(--against-color), rgba(239,68,68,0.5))' 
            }} 
          />
        </div>
        ...
        {isCurrentTurn && (
          <div className="text-[9px] font-display text-gold uppercase tracking-wider animate-pulse flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-gold inline-block" /> Active Turn
          </div>
        )}

        {/* Speech Bubble / Action Panel */}
        <div className="flex-1 flex flex-col justify-center min-h-[160px]">
          {roundArg ? (
            /* Finalized Argument */
            <div className="glass-panel p-4 flex flex-col gap-3 card-speech" style={{ borderLeft: `3px solid ${stanceColor}`, background: 'rgba(255,255,255,0.01)' }}>
              <p className="text-xs text-white/90 leading-relaxed font-body italic" style={{ whiteSpace: 'pre-wrap' }}>
                "{roundArg.argumentText}"
              </p>
              <div className="grid grid-cols-6 gap-1 border-t border-white/5 pt-2 text-center">
                {[
                  { label: 'Topic',    val: roundArg.topicScore },
                  { label: 'Logic',    val: roundArg.factScore },
                  { label: 'Rebuttal', val: roundArg.relatedScore },
                  { label: 'Novelty',  val: roundArg.noveltyScore },
                  { label: 'Stance',   val: roundArg.stanceScore },
                  { label: 'Delivery', val: roundArg.deliveryScore || 0 },
                ].map(({ label, val }) => (
                  <div key={label}>
                    <div className="text-[7px] text-muted-foreground uppercase">{label}</div>
                    <div className="text-[10px] font-bold mt-0.5" style={{ color: val >= 70 ? 'var(--for-color)' : val >= 40 ? 'var(--accent)' : 'var(--against-color)' }}>
                      {val ? val.toFixed(0) : 0}
                    </div>
                  </div>
                ))}
          ...
          ) : isBeingEvaluated && activeEvalData ? (
            /* AI Jury Deliberation waterfall scorecard */
            <div className="glass-panel p-4 bg-black/40 border border-gold/10 rounded-sm">
              <h4 className="font-display text-[10px] text-gold uppercase tracking-wider mb-2 flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 border border-gold border-t-transparent rounded-full animate-spin inline-block" />
                Citadel Jury Deliberating...
              </h4>
              <div className="space-y-1">
                {[
                  { key: 'ddg',    icon: '🔍', label: 'Web Context (RAG)' },
                  { key: 'judge3', icon: '⚖️', label: 'Judge III — Topicality' },
                  { key: 'judge1', icon: '🧠', label: 'Judge I — Logic' },
                  { key: 'judge2', icon: '💥', label: 'Judge II — Rebuttal' },
                  { key: 'judge4', icon: '🎯', label: 'Judge IV — Stance' },
                  { key: 'judge5', icon: '🎙️', label: 'Judge V — Delivery' },
                  { key: 'final',  icon: '🏆', label: 'Compiled Score' },
                ].map(({ key, icon, label }) => {
                  const s = activeEvalData[key];
                  if (!s) return null;
                  return (
                    <div key={key} className="flex justify-between items-center text-[9px] font-body py-0.5 border-b border-white/5 last:border-0">
                      <span className="text-white/70">{icon} {label}</span>
                      <span className="font-bold">
                        {s.status === 'pending' && <span className="text-white/30">Queued</span>}
                        {s.status === 'running' && <span className="text-gold animate-pulse">●</span>}
                        {s.status === 'completed' && <span className="text-emerald-400">✓ {s.score !== null ? s.score : ''}</span>}
                      </span>
                    </div>
                  );
                })}
            ...
            /* Preparing turn placeholder */
            <div className="text-center p-6 border border-white/5 border-dashed rounded-sm text-muted-foreground flex flex-col items-center justify-center gap-2">
              <div className="text-xl animate-bounce">⏳</div>
              <div className="font-display text-[9px] uppercase tracking-wider">House {pName} is preparing argument...</div>
            </div>
          ) : (
            /* Awaiting turn / Guarding the gates */
            <div className="text-center p-6 border border-white/5 rounded-sm text-white/20 flex flex-col items-center justify-center gap-2">
              <div className="text-xl">🛡️</div>
              <div className="font-display text-[9px] uppercase tracking-wider">Guarding the gates...</div>
            </div>
          )}
        </div>
      </div>
  ...
  for (let rNum = 1; rNum <= 3; rNum++) {
    const roundArgs = session.arguments.filter(a => a.roundNumber === rNum);
    let roundWinner = null;
    if (roundArgs.length === 2) {
      if (roundArgs[0].score > roundArgs[1].score) {
        roundWinner = roundArgs[0].userId;
      } else if (roundArgs[1].score > roundArgs[0].score) {
        roundWinner = roundArgs[1].userId;
      }
    }
    roundsList.push({
      roundNumber: rNum,
      ...
      <div style={{ background: 'linear-gradient(180deg, rgba(10,10,11,0.98) 0%, rgba(10,10,11,0.85) 100%)', borderBottom: '1px solid rgba(212,175,55,0.2)', padding: '20px 0 16px' }}>
        <div className="container" style={{ maxWidth: '1200px' }}>
          <div style={{ textAlign: 'center', marginBottom: '16px' }}>
            <div className="medieval" style={{ fontSize: '0.7rem', letterSpacing: '0.25em', color: 'var(--accent)', opacity: 0.75, marginBottom: '6px' }}>⚔ TRIAL BY COMBAT — ROUND {session.currentRound} OF 3 ⚔</div>
      ...
      <div className="container" style={{ maxWidth: '1200px', paddingTop: '28px' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
          {renderPlayerColumn(
            ...
            session.participantA.username,
            hostStance,
            hostHP,
            session.userATotal,
            session.currentTurnUserId === session.participantA.id
          )}
          {renderPlayerColumn(
            session.participantB.id,
            session.participantB.username,
            guestStance,
            guestHP,
            session.userBTotal,
            session.currentTurnUserId === session.participantB.id
          )}
        </div>

        {/* Active Turn Input Deck */}
        {actualIsMyTurn && !streamingProgress && (
        ...
        {/* Spectator Live Wager Console */}
        {isSpectator && (
          <div className="royal-card p-6 mt-6 border border-gold/30 bg-black/40">
            <h3 className="font-display text-sm text-gold uppercase tracking-wider mb-2 flex items-center gap-2">
              <span>🎲</span> Citadel Live Wagering Console
            </h3>
            ...
            {wagerPools ? (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4 text-center">
                  <div className="p-3 bg-white/5 border border-white/5 rounded-sm">
                    <div className="text-[9px] text-muted-foreground uppercase">House {session.participantA.username} (A)</div>
                    <div className="text-sm font-display text-white font-bold mt-1">{wagerPools.sideAPool}G</div>
                    <div className="text-[8px] text-muted-foreground uppercase mt-0.5">{wagerPools.sideABettors} Bettors</div>
                  </div>
                  <div className="p-3 bg-white/5 border border-white/5 rounded-sm">
                    <div className="text-[9px] text-muted-foreground uppercase">House {session.participantB.username} (B)</div>
                    <div className="text-sm font-display text-white font-bold mt-1">{wagerPools.sideBPool}G</div>
                    <div className="text-[8px] text-muted-foreground uppercase mt-0.5">{wagerPools.sideBBettors} Bettors</div>
                  </div>
                </div>
                
                {myActiveBet ? (
                  <div className="p-4 bg-emerald-950/20 border border-emerald-800/30 rounded-sm text-center text-xs text-emerald-400">
                    👑 Your wager of <strong className="text-white">{myActiveBet.stakeAmount}G</strong> is registered on <strong>House {myActiveBet.side === 'A' ? session.participantA.username : session.participantB.username}</strong>.
                  </div>
                ) : wagerPools.isLocked ? (
                  <div className="p-4 bg-red-950/20 border border-red-800/30 rounded-sm text-center text-xs text-red-400">
                    🔒 Wagers are locked for this battle. Arguments have begun.
                  </div>
                ) : (
                  <div className="flex flex-col gap-3">
                    {betError && <div className="p-2.5 bg-red-900/20 border border-red-500/30 text-red-500 rounded-sm text-xs">{betError}</div>}
                    {betSuccess && <div className="p-2.5 bg-emerald-900/20 border border-emerald-500/30 text-emerald-500 rounded-sm text-xs">{betSuccess}</div>}
                    
                    <div className="flex items-center gap-3">
                      <span className="text-xs text-white/70 whitespace-nowrap">Gold Coins Stake:</span>
                      <input 
                        type="number" 
                        min={10} 
                      ...
                      />
                      <span className="text-[9px] text-muted-foreground">(Min: 10G)</span>
                    </div>
                    ...
                    <div className="flex gap-4 mt-1">
                      <button 
                        ...
                        disabled={placingBet}
                        className="btn-gold solid flex-1 !py-2 !text-xs"
                      >
                        Wager on {session.participantA.username}
                      </button>
                      <button 
                        ...
                        className="btn-gold solid flex-1 !py-2 !text-xs"
                      >
                        Wager on {session.participantB.username}
                      </button>
                    </div>
                  </div>
                )}
              ...
              <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground p-4">
                <Icons.Loading />
                <span>Synchronizing wagers with Citadel ledgers...</span>
              </div>
            )}
          </div>
        )}

        {/* Historical Round Archives */}
        <div className="royal-card p-6 mt-6">
          <h3 className="font-display text-sm text-gold uppercase tracking-wider mb-2 flex items-center gap-2">
            <span>📜</span> Citadel Archives — Historical Rounds
          </h3>
          <div className="divider-royal mb-4" />
          
          <div className="space-y-4">
            {roundsList.filter(r => r.roundNumber < session.currentRound).length === 0 ? (
              <div className="text-center p-6 text-xs text-muted-foreground">
                No archives for previous rounds yet.
              </div>
            ) : (
              roundsList.filter(r => r.roundNumber < session.currentRound).map((r) => (
                <div key={r.roundNumber} className="border-b border-white/5 pb-4 last:border-0 last:pb-0">
                  <div className="flex justify-between items-center mb-3">
                    <span className="font-display text-xs text-gold">ROUND {r.roundNumber} ARCHIVES</span>
                    {r.winnerId && (
                      <span className="font-display text-[9px] px-2 py-0.5 rounded-sm uppercase" style={{ color: r.winnerId === 'draw' ? 'var(--accent)' : r.winnerId === session.participantA.id ? 'var(--for-color)' : 'var(--against-color)', background: 'rgba(255,255,255,0.02)' }}>
                        👑 Winner: {r.winnerId === 'draw' ? 'Draw' : r.winnerId === session.participantA.id ? session.participantA.username : session.participantB.username}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {r.arguments.map((arg, idx) => {
                      const isHostArg = String(arg.userId) === String(session.participantA.id);
                      const pStance = isHostArg ? hostStance : guestStance;
                      const stanceColor = pStance === 'FOR' ? 'var(--for-color)' : 'var(--against-color)';
                      return (
                        <div key={idx} className="p-4 bg-black/10 border border-white/5 rounded-sm flex flex-col gap-2">
                          <div className="flex justify-between text-[10px] font-display border-b border-white/5 pb-1.5">
                            <span className="text-white font-semibold">{arg.username} ({pStance})</span>
                            <span className="text-gold font-bold">{arg.score.toFixed(1)} PTS</span>
                          </div>
                          <p className="text-xs text-white/70 leading-relaxed font-body italic">"{arg.argumentText}"</p>
                          <details className="mt-1">
                            <summary className="text-[8px] font-display text-gold/75 cursor-pointer outline-none">▶ View Notes & Web Evidence</summary>
                            <div className="text-[10px] text-muted-foreground bg-black/20 p-2 rounded-sm mt-1.5 space-y-1">
                              <div className="italic">"{arg.reasoning}"</div>
                              {arg.retrievedFacts && <div className="font-mono text-[9px] text-white/50 border-t border-white/5 pt-1 mt-1">🔍 {arg.retrievedFacts}</div>}
                            </div>
                          </details>
                        </div>
                      );
  ...
  // Active player/AI match view: original timeline-based
  return (
    <div className="bg-stone-texture noise-overlay min-h-screen relative" style={{ paddingBottom: '60px' }}>
      {showClash && <div className="clash-fx">⚔</div>}
                          ...
                          return (
                            <div key={key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderRadius: '4px', border: `1px solid ${s.status === 'running' ? 'var(--accent)' : s.status === 'completed' ? 'rgba(16,185,129,0.3)' : 'rgba(255,255,255,0.05)'}`, background: s.status === 'running' ? 'rgba(212,175,55,0.05)' : s.status === 'completed' ? 'rgba(16,185,129,0.03)' : 'transparent', transition: 'all 0.3s ease' }}>
                              ...
                              <div style={{ minWidth: '70px', textAlign: 'right' }}>
                                ...
                                {s.status === 'completed' && <span style={{ color: 'var(--for-color)', fontWeight: '800', fontSize: '0.85rem' }}>{key === 'ddg' ? (s.strategy === 'web' ? 'Search Base' : s.strategy === 'rag' ? 'RAG Context' : 'Internal Knowledge') : (key === 'final' ? `${s.score}` : key === 'judge3' ? `${s.score} / ${s.novelty}` : `${s.score}`)} ✓</span>}
                              </div>
                            </div>
                          );
                        })}
                          ...
                          return (
                            <div key={key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderRadius: '4px', border: `1px solid ${s.status === 'running' ? 'rgba(255,255,255,0.15)' : s.status === 'completed' ? 'rgba(16,185,129,0.3)' : 'rgba(255,255,255,0.04)'}`, background: s.status === 'running' ? 'rgba(255,255,255,0.03)' : 'transparent' }}>
                              ...
                              <div>
                                ...
                                {s.status === 'completed' && (
                                  <span style={{ color: 'var(--for-color)', fontWeight: '800' }}>
                                    ✓ {key === 'ddg' ? (s.strategy === 'web' ? 'Search Base' : s.strategy === 'rag' ? 'RAG Context' : 'Internal Knowledge') : (key === 'final' ? s.score : '')}
                                  </span>
                                )}
                              </div>
                            </div>
...
export function PrivateDebateRoom() {
  ...
  // Sync typed text to other participants over WS in real-time
  useEffect(() => {
    if (!isSpectator && room && room.status === 'ACTIVE' && String(room.current_turn_userId) === String(user.id)) {
      sendAction('typing', { text: argumentText });
    }
  }, [argumentText, room, user.id, isSpectator]);
  ...
  // Calculations for spectator mode
  const isSpectator = room ? (String(user.id) !== String(room.hostUserId) && String(user.id) !== String(room.guestUserId)) : false;

  // Poll active wagers to sync live pools
  useEffect(() => {
    if (!room || !room.dbSessionId || !isSpectator) return;

    const fetchLivePools = async () => {
      try {
        const res = await wagerApi.getActiveWagers();
        const myMatch = res.find(m => String(m.id) === String(room.dbSessionId) || m.roomCode === room.roomCode);
        if (myMatch) {
          setWagerPools(myMatch);
        }
      } catch (e) {
  ...
  useEffect(() => {
    ...
    const fetchUserBetHistory = async () => {
      try {
        const history = await wagerApi.getHistory();
        const existingBet = history.find(b => String(b.matchId) === String(room.dbSessionId));
        if (existingBet) {
          setMyActiveBet(existingBet);
        }
      } catch (e) {
  ...
  const handlePlaceWager = async (side) => {
    ...
    setPlacingBet(true);
    try {
      const res = await wagerApi.placeBet(room.dbSessionId, side, betAmount);
      setBetSuccess("Wager registered in the Citadel ledger!");
      // Reload history
      const history = await wagerApi.getHistory();
      const existingBet = history.find(b => String(b.matchId) === String(room.dbSessionId));
      ...
      // Reload pools
      const pools = await wagerApi.getActiveWagers();
      const myMatch = pools.find(m => String(m.id) === String(room.dbSessionId) || m.roomCode === room.roomCode);
      if (myMatch) {
        setWagerPools(myMatch);
      }
    } catch (err) {
          ...
          setStreamingProgress(prev => {
            ...
            switch (ev) {
              case 'ddg_start':
                ...
                break;
              case 'ddg_complete':
                next.ddg = { status: 'completed', detail: payload.message, strategy: payload.strategy };
                break;
              case 'judge3_start':
  ...
  const isMyTurn = room.status === 'ACTIVE' && String(room.current_turn_userId) === String(user.id);
  ...
  if (room.status === 'ACTIVE' || streamingProgress) {
    if (isSpectator) {
    const hostStance = room.user_sides[room.hostUserId] || 'FOR';
    const guestStance = room.guestUserId ? (room.user_sides[room.guestUserId] || 'AGAINST') : 'AGAINST';
    
    // Calculate dynamic combat HP based on round margins
    let hostHP = 100;
    let guestHP = 100;
    if (room && room.arguments) {
      const roundArgs = {};
      ...
      Object.values(roundArgs).forEach(args => {
        if (args.length === 2) {
          ...
          let scoreB = 0; // Guest
          args.forEach(arg => {
            if (String(arg.userId) === String(room.hostUserId)) scoreA = arg.score;
            else scoreB = arg.score;
          });
          if (scoreA > scoreB) {
            guestHP -= (scoreA - scoreB);
    ...
    const hostTotal = room.arguments.filter(a => String(a.userId) === String(room.hostUserId)).reduce((sum, a) => sum + a.score, 0);
    const guestTotal = room.guestUserId ? room.arguments.filter(a => String(a.userId) === String(room.guestUserId)).reduce((sum, a) => sum + a.score, 0) : 0;

    // Render a player's active column helper
    const renderPlayerColumn = (pId, pName, pStance, pHP, pScore, isCurrentTurn) => {
      const roundArg = room.arguments.find(a => a.roundNumber === room.current_round && String(a.userId) === String(pId));
      const draftText = room.drafts ? room.drafts[pId] : '';
      
      const roundArgsForRound = room.arguments.filter(a => a.roundNumber === room.current_round);
      const isBeingEvaluated = streamingProgress && (
        (roundArgsForRound.length === 0 && String(pId) === String(room.hostUserId)) ||
        (roundArgsForRound.length === 1 && String(pId) === String(room.guestUserId))
      );
      
      const isTyping = isCurrentTurn && draftText && draftText.trim().length > 0;
        ...
        <div className={`royal-card p-6 flex flex-col gap-4 relative transition-all duration-300 ${isCurrentTurn ? 'ring-2 ring-gold' : 'border-white/5 bg-black/20'}`} style={{ border: isCurrentTurn ? '1px solid var(--accent)' : undefined }}>
          ...
          <div className="flex items-center justify-between border-b border-white/5 pb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-display text-sm text-white font-semibold flex items-center gap-1.5">
                  {pName}
                  {String(pId) === String(room.hostUserId) && <span className="text-[10px] text-gold font-normal">(Host)</span>}
                </div>
                <div className="font-display text-[9px] px-2 py-0.5 rounded-sm uppercase tracking-wider inline-block mt-1" style={{ color: stanceColor, background: stanceBg, border: `1px solid ${stanceColor}20` }}>
                  House {pStance}
                </div>
          ...
          <div className="flex-1 flex flex-col justify-center min-h-[160px]">
            {roundArg ? (
                        ...
                        <span className="font-bold">
                          ...
                          {s.status === 'completed' && (
                            <span className="text-emerald-400">
                              ✓ {key === 'ddg' ? (s.strategy === 'web' ? 'Search Base' : s.strategy === 'rag' ? 'RAG' : 'Internal') : (s.score !== undefined && s.score !== null ? s.score : '')}
                            </span>
                          )}
                        </span>
                      </div>
              ...
              /* Live Typing Preview */
              <div className="glass-panel p-4 bg-gold/[0.02] border border-gold/20 rounded-sm min-h-[120px] flex flex-col justify-between">
                <p className="text-xs text-white/80 leading-relaxed font-body italic">
                  "{draftText}"
                </p>
                <div className="text-[8px] font-display text-gold/60 uppercase tracking-widest mt-2 flex items-center gap-1">
                  <span className="w-1 h-1 rounded-full bg-gold animate-ping" /> Writing argument...
                </div>
              </div>
            ) : isCurrentTurn ? (
              /* Preparing turn placeholder */
    ...
    return (
      <div className="bg-stone-texture noise-overlay min-h-screen relative" style={{ paddingBottom: '60px' }}>
        ...
        <div style={{ background: 'linear-gradient(180deg, rgba(10,10,11,0.98) 0%, rgba(10,10,11,0.85) 100%)', borderBottom: '1px solid rgba(212,175,55,0.2)', padding: '20px 0 16px' }}>
          <div className="container" style={{ maxWidth: '1200px' }}>
            <div style={{ textAlign: 'center', marginBottom: '16px' }}>
              <div className="medieval" style={{ fontSize: '0.7rem', letterSpacing: '0.25em', color: 'var(--accent)', opacity: 0.75, marginBottom: '6px' }}>⚔ TRIAL BY COMBAT — ROUND {room.current_round} OF {room.rounds} ⚔</div>
              <h1 className="heading text-gold-gradient" style={{ fontSize: 'clamp(1.1rem, 3vw, 1.6rem)', lineHeight: 1.3 }}>{room.topic}</h1>
            </div>
          </div>
        </div>
        ...
        <div className="container" style={{ maxWidth: '1200px', paddingTop: '28px' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
            {renderPlayerColumn(
              ...
              hostHP,
              hostTotal,
              String(room.current_turn_userId) === String(room.hostUserId)
            )}
            {renderPlayerColumn(
              room.guestUserId || 'guest',
              room.guestUsername || 'Awaiting...',
              guestStance,
              guestHP,
              guestTotal,
              room.guestUserId && String(room.current_turn_userId) === String(room.guestUserId)
            )}
          </div>

          {/* Active Turn Input Deck */}
          {isMyTurn && !streamingProgress && (
            <div className="royal-card p-6 mt-6">
              <form onSubmit={(e) => {
                e.preventDefault();
                ...
                sendAction('submit_argument', { argumentText, rawTranscript: finalRawTranscript || argumentText });
                setArgumentText('');
                recordedWordsRef.current = [];
                lastProcessedIndexRef.current = 0;
            ...
            <div className="royal-card p-6 mt-6 border border-gold/30 bg-black/40">
              ...
              {wagerPools ? (
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4 text-center">
                    <div className="p-3 bg-white/5 border border-white/5 rounded-sm">
                      <div className="text-[9px] text-muted-foreground uppercase">House {room.hostUsername} (A)</div>
                      <div className="text-sm font-display text-white font-bold mt-1">{wagerPools.sideAPool}G</div>
                      <div className="text-[8px] text-muted-foreground uppercase mt-0.5">{wagerPools.sideABettors} Bettors</div>
                    </div>
                    <div className="p-3 bg-white/5 border border-white/5 rounded-sm">
                      <div className="text-[9px] text-muted-foreground uppercase">House {room.guestUsername || 'Guest'} (B)</div>
                      <div className="text-sm font-display text-white font-bold mt-1">{wagerPools.sideBPool}G</div>
                      <div className="text-[8px] text-muted-foreground uppercase mt-0.5">{wagerPools.sideBBettors} Bettors</div>
                    </div>
                  </div>
                  ...
                  {myActiveBet ? (
                    <div className="p-4 bg-emerald-950/20 border border-emerald-800/30 rounded-sm text-center text-xs text-emerald-400">
                      👑 Your wager of <strong className="text-white">{myActiveBet.stakeAmount}G</strong> is registered on <strong>House {myActiveBet.side === 'A' ? room.hostUsername : room.guestUsername}</strong>.
                    </div>
                  ) : wagerPools.isLocked ? (
                    <div className="p-4 bg-red-950/20 border border-red-800/30 rounded-sm text-center text-xs text-red-400">
                      🔒 Wagers are locked for this battle. Arguments have begun.
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <div className="flex gap-4 mt-1">
                        ...
                        <button 
                          ...
                          className="btn-gold solid flex-1 !py-2 !text-xs"
                        >
                          Wager on {room.guestUsername || 'Guest'}
                        </button>
                      </div>
                    </div>
                  )}
          ...
          <div className="royal-card p-6 mt-6">
            ...
            <div className="space-y-4">
              {roundsList.filter(r => r.roundNumber < room.current_round).length === 0 ? (
                <div className="text-center p-6 text-xs text-muted-foreground">
                  No archives for previous rounds yet.
                </div>
              ) : (
                roundsList.filter(r => r.roundNumber < room.current_round).map((r) => (
                  <div key={r.roundNumber} className="border-b border-white/5 pb-4 last:border-0 last:pb-0">
                    <div className="flex justify-between items-center mb-3">
                      <span className="font-display text-xs text-gold">ROUND {r.roundNumber} ARCHIVES</span>
                      {r.winnerId && (
                        <span className="font-display text-[9px] px-2 py-0.5 rounded-sm uppercase" style={{ color: r.winnerId === 'draw' ? 'var(--accent)' : r.String(winnerId) === String(room.hostUserId) ? 'var(--for-color)' : 'var(--against-color)', background: 'rgba(255,255,255,0.02)' }}>
                          👑 Winner: {r.winnerId === 'draw' ? 'Draw' : r.String(winnerId) === String(room.hostUserId) ? room.hostUsername : room.guestUsername}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {r.arguments.map((arg, idx) => {
                        const isHostArg = String(arg.userId) === String(room.hostUserId);
                        const pStance = isHostArg ? hostStance : guestStance;
                        const stanceColor = pStance === 'FOR' ? 'var(--for-color)' : 'var(--against-color)';
                        return (
                          <div key={idx} className="p-4 bg-black/10 border border-white/5 rounded-sm flex flex-col gap-2">
